...
describe('[Görev 6] Araba', () => {
  ...
  beforeEach(() => {
   ...
   test('[15] arabayı sürünce güncellenmiş odometer döndürüyor', () => {
     let totalNumber = focus.sur(100)
     totalNumber = focus.sur(100)
     totalNumber = focus.sur(100)
     totalNumber = focus.sur(200)
     totalNumber = focus.sur(200)
     expect(totalNumber).toBe(600)
   })
...
describe('[Görev 7] asenkronCiftSayi', () => {
   test('[19] bir çift sayı verilirse true çözümlüyor', async () => {
     let isCift = await utils.asenkronCiftSayi(2)
     expect(isCift).toBe(true)
   })
   test('[20] tek sayı verilirse false çözümlüyor', async () => {
    let isCift = await utils.asenkronCiftSayi(1)
    expect(isCift).toBe(false)
   })
})
